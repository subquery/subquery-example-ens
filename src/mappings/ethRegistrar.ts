...
import {
  NameRegisteredEvent as ControllerNameRegisteredEventOld
} from '../types/ethers-contracts/EthRegistrarControllerOld'
...
import { Account, Domain, NameRegistered, NameRenewed, NameTransferred, Registration } from '../types/models'
import {EthereumLog} from "@subql/types-ethereum";
import {keccak256} from "@ethersproject/keccak256"

var rootNode = byteArrayFromHex("93cdeb708b7545dc668eb9280176169d1c33cfd8ed6f04690a0bcc88a93fc4ae")

export async function handleNameRegistered(event: EthereumLog<NameRegisteredEvent["args"]>): Promise<void> {
  ...
  let domain = await Domain.get(keccak256(concat(rootNode.toString(), label.toString())))
...
export async function handleNameRegisteredByControllerOld(event: ControllerNameRegisteredEventOld): Promise<void> {
  await setNamePreimage(event.args.name, event.args.label, event.args.cost.toBigInt());
}

export async function handleNameRegisteredByController(event: ControllerNameRegisteredEvent): Promise<void> {
  await setNamePreimage(event.args.name, event.args.label, (event.args.baseCost.add(event.args.premium)).toBigInt())
}

export async function handleNameRenewedByController(event: EthereumLog<ControllerNameRenewedEvent["args"]>): Promise<void> {
  await setNamePreimage(event.args.name, event.args.label, event.args.cost.toBigInt());
}
...
function checkValidLabel(name: string): boolean {
  for (let i = 0; i < name.length; i++) {
    ...
    if (c === 0) {
      ...
      logger.warn("Invalid label '{}' contained separator char '.'. Skipping.", [name]);
      return false;
    }
  }
...
async function setNamePreimage(name: string, label: string, cost: bigint): Promise<void> {
...
export async function handleNameRenewed(event: EthereumLog<NameRenewedEvent["args"]>): Promise<void> {
  let label = uint256ToByteArray(event.args.id.toHexString())
...
export async function handleNameTransferred(event: EthereumLog<TransferEvent["args"]>): Promise<void> {
